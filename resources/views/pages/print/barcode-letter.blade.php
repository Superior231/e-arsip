...
@section('content')
    <nav class="sticky-top container-fluid d-flex justify-content-between py-3 px-2 px-md-5 W-100 mb-3">
        <a href="{{ route('index') }}" class="back d-flex align-items-center text-decoration-none text-dark fw-bold gap-1">
            <i class="bx bx-chevron-left fs-3 fw-bold text-dark"></i>
    ...
    <div id="areaPrint">
        <div class="barcode-container">
            @forelse ($letters as $letter)
                <div class="barcode">
                    <div class="img">
                        <img src="https://api.qrserver.com/v1/create-qr-code/?data={{ request()->getHost() }}/print/letter/{{ $letter->no_letter }}&size=100x100" alt="QR Code">
                    </div>
                    <div class="title">
                        <span>{{ $letter->letter_code }} - {{ $letter->name }}</span>
                    </div>
                </div>
            @empty
                <div class="d-flex align-items-center justify-content-center w-100">
                    <span>Surat tidak ada.</span>
                </div>
            @endforelse
        </div>
    </div>
